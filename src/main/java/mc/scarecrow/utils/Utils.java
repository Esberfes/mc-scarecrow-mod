...
public abstract class Utils {
    ...
    public static Optional<Chunk> getLoadedChunk(World world, BlockPos pos)
    {
        ...
        return Optional.ofNullable(world.getChunkProvider().getChunk(pos.getX() >> 4, pos.getZ() >> 4, true));
    }
     ...
     */
    private static boolean checkHeight(final World world, final int x, final int y, final int z, final int height, final Block... blocks) {
     ...
     */
    public static boolean isBlockInRange(final World world, final Block block, final int posX, final int posY, final int posZ, final int range) {
    ...
    public static void checkDirectory(final File directory) {
        if (!directory.exists() && !directory.mkdirs()) {
            LogManager.getLogger().error("Directory doesn't exist and failed to be created: " + directory.toString());
        }
    }
